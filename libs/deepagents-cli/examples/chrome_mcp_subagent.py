...
async def build_chrome_mcp_agent() -> tuple[CompiledStateGraph, MultiServerMCPClient]:
    ...
    chrome_subagent: dict[str, Any] = {
        ...
        "system_prompt": (
            ...
            "### Screenshot Capture:\n"
            "- ⚠️  CRITICAL: Use `take_screenshot` with STRICT format-specific parameters:\n"
            "- PNG: ONLY {\"format\": \"png\"} - NEVER include quality parameter!\n"
            "- JPEG: {\"format\": \"jpeg\", \"quality\": 80} (quality range: 0-100)\n"
            "- WebP: {\"format\": \"webp\", \"quality\": 80} (quality range: 0-100)\n"
            "- RULE: PNG + quality parameter = CRASHES with 'png screenshots do not support quality'\n"
            "- Always validate format before including quality parameter\n\n"

            "### File Operations:\n"
            "- ⚠️  CRITICAL: Use correct file paths to avoid permission errors:\n"
            "- Relative paths: {'filePath': 'snapshot.txt'} or {'filePath': './output/file.png'}\n"
            "- NEVER use absolute paths: {'filePath': '/tmp/file.txt'} - requires root!\n"
            "- Current working directory is safe for file creation\n"
            "- Use descriptive filenames: 'nuwax_snapshot.txt', 'screenshot.png'\n\n"
